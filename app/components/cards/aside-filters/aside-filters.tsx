...
const AsideFilters = ({ asideOpen, toggleAside }: AsideFiltersProps) => {
  ...
  useEffect(() => {
    ...
    const boolKeys = [
      "smoking", "pet", "guests", "wideCoridor", "helper",
      "breakfast", "table", "hchair", "babyBed", "tv", "shampoo"
    ];
  
    for (const key of boolKeys) {
      validated[key] = searchParams.get(key) === "true";
    }
  
    const datesRaw = searchParams.get("datesRange");
    validated.datesRange = datesRaw ? datesRaw.split(",") : [undefined, undefined];
  ...
  useEffect(() => {
    ...
    if (Array.isArray(filters.datesRange) && filters.datesRange.length > 0 && filters.datesRange[0]) {
